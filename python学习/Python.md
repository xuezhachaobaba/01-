...
#### python3解释器
python3解释器是一种用于执行python3代码的程序，它的核心作用是将人类可读的python源代码转换为计算机能够理解和执行的机器指令。
简单来说，当你编写了一段python3代码（通常保存为.py文件），需要通过python3解释器才能运行这段代码。与编译型语言（c、java）先将整个代码编译成机器码再执行不同，python解释器采用逐行解释执行的方式：读一行代码，解析并执行一行，直到程序结束。
#### python运算符
##### 什么是运算符？
+、-、\*、/都是运算符
python语言支持以下类型的运算符：
...
##### 比较运算符
![[Pasted image 20250914144427.png]]
##### 赋值运算符
![[Pasted image 20250914144715.png]]
在python3.8以及更高的版本中，引入了一种新的语法特性，成为”海象运算符“walrus operator，它使用:=符号。这个运算符的主要目的是在表达式中同时进行赋值和返回赋值的值。
...
2. 在上述例子中，传统写法需要单独一行来赋值n，然后用if语句进行条件检查。而使用海象运算符的写法可以在if语句中直接进行赋值和条件检查。
##### 位运算符
